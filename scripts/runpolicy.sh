#!/bin/bash
# Run the trained ACT policy on the SO101 follower (no teleop leader).
# Run: source scripts/setup.sh && ./scripts/runpolicy.sh
...
RUN_REPO_ID="${RUN_REPO_ID:-${HF_USER}/run_act_desk_trash_$(date +%Y%m%d_%H%M%S)}"
...
echo "Running policy from ${POLICY_PATH}"
echo "Logging episodes to ${RUN_REPO_ID}"
